...
app.get('/mahasiswa/:nim', (req, res) => {
    const qSQL = `SELECT * FROM mahasiswa WHERE nim = ${db.escape(req.params.nim)}`
    ...
    db.query(qSQL, (err, result) => {
        ...
        if (result.length > 0) {
            response(200, `SUCCESSFULLT GET MAHASISWA DATA WHERE NIM IS ${req.params.nim}`, result, res)
        } else {
            response(404, `CANNOT FIND MAHASISWA DATA WHERE NIM IS ${req.params.nim}`, null, res)
        }
    })
})
...
app.get('/find', (req, res) => {
    if (typeof req.query.nim == 'undefined' || req.query.nim.length == 0) {
        ...
        const qSQL = `SELECT * FROM mahasiswa WHERE nim = ${db.escape(req.query.nim)}`
        db.query(qSQL, (err, result) => {
            ...
            if (result.length > 0) {
                const message = "SUCCESSFULLY FETCH MAHASISWA DATAS WHERE NIM IS " + req.query.nim 
                response(200, message, result, res)
            } else {
